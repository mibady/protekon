  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-8">
      ...
      <div>
        ...
        <h1 className="font-display font-bold text-[24px] lg:text-[32px] text-parchment mt-2">
          Extend PROTEKON
        </h1>
      ...
      {/* Integrations */}
      <section>
        <h2 className="font-display font-bold text-[11px] tracking-[4px] uppercase text-gold mb-5">
          Integrations
        </h2>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {integrations.map((integration, i) => (
            <motion.div
              ...
              className="bg-midnight/50 border border-brand-white/[0.06] p-5 rounded-lg hover:border-gold/30 transition-colors"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.4, delay: i * 0.05 }}
            >
              <div className="flex items-start justify-between mb-3">
                <div className="w-10 h-10 flex items-center justify-center bg-gold/10 rounded">
                  <integration.icon size={20} weight="bold" className="text-gold" />
                </div>
              ...
              <h3 className="font-display font-bold text-[15px] text-parchment mb-1">
                {integration.name}
              </h3>
              <span className="font-display text-[9px] tracking-[2px] uppercase text-crimson block mb-2">
      ...
      <section>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {addOns.map((addon, i) => (
            <motion.div
              key={addon.name}
              className="bg-midnight/50 border border-brand-white/[0.06] p-6 rounded-lg"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              <div className="flex items-start justify-between mb-3">
                ...
                <span className="font-display font-bold text-[18px] text-gold">
                  {addon.price}
                </span>
              </div>
              ...
              <button className="w-full font-display font-semibold text-[10px] tracking-[3px] uppercase text-parchment border border-brand-white/20 py-3 rounded hover:border-gold hover:text-gold transition-colors">
                Add to Plan
              </button>
            </motion.div>
          ))}
      ...
      <section>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {partners.map((partner, i) => (
            <motion.div
              key={partner.name}
              className="bg-midnight/50 border border-brand-white/[0.06] p-5 rounded-lg text-center"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.4, delay: i * 0.05 }}
            >
              <h3 className="font-display font-bold text-[13px] text-parchment mb-1">
                {partner.name}
              </h3>
      ...
      {/* CTA */}
      <section className="bg-crimson/10 border border-crimson/20 rounded-lg p-6 sm:p-8 text-center">
        <Clock size={36} weight="light" className="text-crimson mx-auto mb-4" />
        <h2 className="font-display font-bold text-[20px] lg:text-[24px] text-parchment mb-3">
          Need a Custom Integration?
        </h2>
        <p className="font-sans text-[14px] text-steel mb-5">
          Our API team can build custom integrations for enterprise customers.
        </p>
        <button className="font-display font-semibold text-[10px] tracking-[3px] uppercase text-parchment bg-crimson px-8 py-3 rounded hover:brightness-110 transition-all">
          Contact API Team
        </button>
      </section>
    </div>
